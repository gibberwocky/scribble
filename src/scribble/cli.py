...
""",
    ...
    # ------------- Pseudotime -------------
    pseudotime_parser = subparsers.add_parser("pseudotime",
        help="Infer lineage pseudotime using scFates")

    pseudotime_parser.add_argument("--project_dir", required=True)
    pseudotime_parser.add_argument("--input", required=True,
        help="Annotated refined atlas")
    pseudotime_parser.add_argument("--clusters", nargs="+", required=True,
        help="Ordered lineage clusters")
    pseudotime_parser.add_argument("--root", default=None,
        help="Root cluster (defaults to first cluster)")
    pseudotime_parser.add_argument("--celltype", default="cell_type_minor")
    pseudotime_parser.add_argument("--batch", default="sample",
        help="Batch column used for Harmony if >1 sample")
    pseudotime_parser.add_argument("--hvgs", type=int, default=3000)
    pseudotime_parser.add_argument("--npcs", type=int, default=50)
    pseudotime_parser.add_argument("--neighbors", type=int, default=20)
    pseudotime_parser.add_argument("--nodes", type=int, default=10,
        help="Number of principal graph nodes (epigraph)")
    pseudotime_parser.add_argument("--tf_list", default=None)
    ...
    premonod_parser = subparsers.add_parser(
        "premonod",
        description="""
    Prepare data for analysis with monod.

    An annotated AnnData file is written with the suffix
    '_monod.h5ad'.
    """
    )
    premonod_parser.add_argument("--project_dir", required=True,
        help="Project directory containing the Scribble output folders")
    premonod_parser.add_argument("--velocyto_dir", required=True,
        help="Directory containing the velocyto files for the samples")
    premonod_parser.add_argument("--input", required=True,
        help="Input AnnData (.h5ad) file generated by Scribble")
    premonod_parser.add_argument("--samples", nargs="+", required=True,
        help="Sample names matching velocyto and loom subdirectories and sample names in AnnData")
    premonod_parser.add_argument("--clusters", nargs="+", required=True,
        help="Clusters to be retained for Monod analysis")
    premonod_parser.add_argument("--min_shared_counts", type=int, default=20,
        help="Minimum shared spliced/unspliced counts per gene")
    ...
    COMMANDS = {
        ...
        "pseudotime": ("scribble.pseudotime", "run_pseudotime")
    }
